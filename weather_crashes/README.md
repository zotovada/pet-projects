...
- построения аналитических панелей в BI-системах (Tableau, DataLens, Power BI) или средствами Python (matplotlib, seaborn, plotly, dash, streamlit).
...
- Скрипты используют абсолютные импорты через `src` (`from src import ...`).
- Все локальные данные находятся в папке `data/`.
- Перед первым запуском убедитесь, что Supabase настроен и ключи в `config.py` корректны.